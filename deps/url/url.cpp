...
static const char* supported_scheme[] = {"http", "https"};
...
bool check_scheme(const std::string& s) {
    for (auto scheme: supported_scheme) {
        if(s == scheme) {
            return true;
        }
    }
...
void Url::parse_url() const {
    ...
    std::int8_t ip_v=-1;
    const char *user_b, *user_e, *host_b, *host_e,
            *port_b, *port_e, *path_b, *path_e, *query_b, *query_e,
            *fragment_b, *fragment_e;
    user_b=user_e=host_b=host_e=port_b=port_e=path_b=
    ...
    if (p==e) {
        ...
        if (*p==':') {
            ...
            _scheme=normalize_scheme(b, p);
            if(!check_scheme(_scheme)) {
                throw Url::parse_error("Scheme in '"+std::string(s,e-s)+"' is unsupported");
            }
